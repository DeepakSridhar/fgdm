...
CrossAttentionLayers_XL = [
    'diffusion_model.input_blocks.2.1.transformer_blocks.3.attn2',
    'diffusion_model.input_blocks.2.1.transformer_blocks.4.attn2',
    'diffusion_model.middle_block.0.transformer_blocks.0.attn2',
    'diffusion_model.middle_block.0.transformer_blocks.1.attn2',
    'diffusion_model.middle_block.0.transformer_blocks.2.attn2',
    'diffusion_model.middle_block.0.transformer_blocks.3.attn2',
    'diffusion_model.output_blocks.0.0.transformer_blocks.1.attn2',
    'diffusion_model.output_blocks.0.0.transformer_blocks.2.attn2',
    'diffusion_model.output_blocks.0.0.transformer_blocks.3.attn2',
    'diffusion_model.output_blocks.0.0.transformer_blocks.4.attn2',
    'diffusion_model.output_blocks.0.0.transformer_blocks.5.attn2',
    'diffusion_model.output_blocks.0.0.transformer_blocks.6.attn2',
    'diffusion_model.output_blocks.0.0.transformer_blocks.7.attn2',
    'diffusion_model.output_blocks.1.0.transformer_blocks.0.attn2'
]
...
def get_token_maps(selfattn_maps, crossattn_maps, n_maps=1, save_dir='tmp', width=32, height=32, resn=32, obj_tokens=None, seed=0, tokens_vis=None,
    ...
    attn_maps_1024 = torch.cat([torch.stack(v).mean(0)
    ...
    for attn_map in crossattn_maps.values():
        # print("cross", attn_map.shape)
        bs = attn_map.shape[0]
        resolution_map = np.sqrt(attn_map.shape[1]).astype(int)
        attn_map = attn_map.reshape(
            bs, resolution_map, resolution_map, -1).permute([0, 3, 1, 2]).float()
        attn_map = torch.nn.functional.interpolate(attn_map, (resolution, resolution),
                                                   mode='bicubic', antialias=True)
        cross_attn_maps_1024.append(attn_map.permute([0, 2, 3, 1]))

    cross_attn_maps_1024 = torch.stack(
    ...
    if return_vis:
        return attn_maps_1024, cross_attn_maps_1024, segments_vis
    else:
        return attn_maps_1024, cross_attn_maps_1024
