...
from ldm.data.simple_transform import SimpleTransform
...
class CustomDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self._preset_cfg = self._cfg['PRESET']
        self._root = self._cfg['ROOT']
        self._img_prefix = self._cfg['IMG_PREFIX']
        self._ann_file = list(self._cfg['ANN'])
        self._num_datasets = 1
        self.pose_only = pose_only
        self.use_float16 = use_float16
        self.person_only = self._cfg['person_only']
        print(self.person_only, self._use_pose, self.pose_only)
    ...
    def __getitem__(self, idx):
        ...
        img, label, label_mask, bbox, joints = self.transformation(img, label)
        return img, label, label_mask, img_id, bbox, joints
