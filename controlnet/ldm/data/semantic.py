...
class ImageDataset(Dataset):
    def __init__(
        ...
        self.alternate_captions = alternate_captions
        ...
        # if self.use_canny:
        #     self.model_canny = CannyDetector()     
    ...
    def __getitem__(self, idx):
        ...
        if self.local_poses is not None:
            ...
            # center = np.array([center_w, center_h])
            try:
            ...
            pose_path = self.local_poses[idx] # DEBUG: from classes to instances, may affect CelebA
        ...
        if self.dataset_mode == 'cityscapes':
            ...
            if self.is_train:
                if self.random_crop:
                    ...
                    arr_image, arr_class, arr_instance, arr_pose = center_crop_arr([pil_image, pil_class, pil_instance, pil_pose], self.resolution)
            else:
        ...
        if self.dataset_mode == 'sample':
            ...
            # arr_class2 = decolorize(arr_class)
            # assert (tmp == arr_class2).all()
        ...
        if arr_pose is not None:
            ...
            out_dict['pose'] = arr_pose #np.transpose(arr_image, [2, 0, 1])

        arr_class = arr_class.astype(np.float32) / 127.5 - 1
        
        out_dict['label'] = arr_class #np.transpose(arr_image, [2, 0, 1])
        ...
        out_dict["caption"] = classes_str
        if self.alternate_captions:
            if random.random() >= 0.5:
                out_dict["caption"] = classes_str
            else:
                out_dict["caption"] = self.captions[path]
        else:
            if self.label_captions:
                out_dict["caption"] = classes_str
            else:
                out_dict["caption"] = self.captions[path]
        return out_dict
